...
- README.md states the project's provenance and purpose: the code, tests,
  workflows and documents were written by Claude from a brief, for the
  gitops-golden-path course, with the design decisions made and reviewed by
  blairforce1. From this change on, every generated commit carries a
  `Co-Authored-By` trailer naming its model. The specification and the
  decision records are under `docs/`. Nothing about the formatter changed.
